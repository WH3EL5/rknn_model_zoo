...
def decode_yolov5_head(output: np.ndarray, anchors: np.ndarray, input_size):
    ...
    class_scores = feat[:, 5:, :, :].transpose(0, 2, 3, 1).reshape(-1, no - 5)

    return boxes_xyxy, objectness, class_scores
...
def post_process(outputs, input_size, obj_thresh, nms_thresh):
    ...
    # Multi-output head mode: e.g. [(1,255,80,80), (1,255,40,40), (1,255,20,20)].
    if len(outputs) >= 3 and all(o.ndim == 4 for o in outputs[:3]):
        outputs_sorted = sorted(outputs[:3], key=lambda x: x.shape[-1], reverse=True)
        ...
        for i, out in enumerate(outputs_sorted):
            ...
            boxes_i, obj_i, cls_i = decode_yolov5_head(out, YOLOV5_ANCHORS[i], input_size)
