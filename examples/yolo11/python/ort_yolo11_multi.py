...
def post_process(outputs, input_size, obj_thresh, nms_thresh):
    ...
    # Branch mode (YOLO11 optimized ONNX): [box_dfl, cls, score_sum] x 3 scales.
    # score_sum is used for quick filtering only, final score uses per-class confidence.
    if len(outputs) >= 6 and outputs[0].ndim == 4:
        boxes_all, cls_all, score_sum_all = [], [], []
        default_branch = 3
        ...
        for i in range(default_branch):
            ...
            if pair_per_branch >= 3:
                score_sum_all.append(outputs[pair_per_branch * i + 2])
            else:
                score_sum_all.append(np.ones_like(outputs[pair_per_branch * i + 1][:, :1, :, :], dtype=np.float32))
        ...
        score_sum = np.concatenate([flatten_per_level(v) for v in score_sum_all]).reshape(-1)

        classes = np.argmax(classes_conf, axis=1)
        class_scores = classes_conf[np.arange(classes_conf.shape[0]), classes]
        scores = class_scores

        # For YOLO11 optimized graph, score_sum approximates confidence sum and is used as prefilter.
        keep_mask = (scores >= obj_thresh) & (score_sum >= obj_thresh)
        ...
        if pred.ndim != 2:
            raise ValueError(f"Unexpected YOLO output shape: {outputs[0].shape}")

        if pred.shape[0] < pred.shape[1]:
            pred = pred.T

        if pred.shape[1] <= 4:
            raise ValueError(f"Invalid YOLO prediction shape after transpose: {pred.shape}")
...
def main():
    parser = argparse.ArgumentParser("YOLO11 ONNX Runtime inference")
    parser.add_argument(
        "--model",
        type=str,
        required=True,
    ...
    detector = Yolo11OrtDetector(model_path, providers=providers, fallback_input_size=args.input_size)
...
# python python/ort_yolo11_multi.py --model model/yolo11n.onnx --source model/bus.jpg --save_dir python/result_ort --providers CPUExecutionProvider